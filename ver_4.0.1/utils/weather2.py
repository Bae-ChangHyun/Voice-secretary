...
def get_weather(user_input):
    if '내일' in user_input: date = '내일'
    elif '모레' in user_input: date = '모레'
    else: date = '오늘'
    ...
    while(1):
        try:
            ...
            tts_model.speak(f"{nowDate} {loc.text}의 {date} 기상정보입니다. {min_temps.text} {max_temps.text}이며, {temps.text}입니다. 오전 강수확률은 {am_rains.text}이고, 오후 강수확률은 {pm_rains.text}이며 미세먼지는 {dusts.text}입니다. 전체적으로 어제보다 {summary.text[:-2]}습니다.")
            ...
            tts_model.speak("죄송합니다. 제가 잘못된 정보를 찾은 것 같습니다. 다시 한번 말씀해주세요.")
